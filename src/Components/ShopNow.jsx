...
function ShopNow() {
  ...
  const [totalDub, setTotalDub] = useState(128000000000);
  ...
  const handleChange = (item, d) => {
    const ind = cartProduct.indexOf(item);
...
const exic=cartProduct.filter((item)=>item.id !== cart.id)
setCartProduct(exic)
  ...
  return (
    <div>
      <Header total={totalDub} />   
           <div className="proLayer" >
      {product.map((e, i) => (

          <ProductItem element={e} key={i} count={count} addCart={addToCart} />
      
      ))}
  </div>
      <Cart cart={cartProduct} handleChange={handleChange}  handleDelete={handleDelete}/>
    </div>
  );
}
